        ...
        else
        {
            ...
            for (int i = 0; i < ret; i++)
            {
                ...
                else
                {
                    ...
                    else if (epevs[i].events & EPOLLIN) // Use else if instead of else
                    {
                        char recvBuf[5];
                        int bytesRead = 0;
                        while (1)
                        {
                            int len = read(curfd, recvBuf + bytesRead, sizeof(recvBuf) - bytesRead);
                            if (len == -1)
                            {
                        ...
                        if (bytesRead > 0)
                        {
                            write(STDOUT_FILENO, recvBuf, bytesRead);
                            write(curfd, recvBuf, bytesRead);
                        }
                    }
                }
            }
